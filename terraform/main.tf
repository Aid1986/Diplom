...
resource "yandex_compute_instance" "web-server-nginx-1" {
  ...
  network_interface {
    ...
    security_group_ids = [yandex_vpc_security_group.andrey-private.id]
  }
  metadata = {
...
resource "yandex_compute_instance" "zabbix" {
  ...
  network_interface {
    ...
    security_group_ids = [yandex_vpc_security_group.andrey-public.id]
  }
  metadata = {
    user-data = "${file("./meta.txt")}"
  }
...
resource "yandex_compute_instance" "bastion-host" {
  ...
  network_interface {
    ...
    nat                = true
    security_group_ids = [yandex_vpc_security_group.andrey-gw.id]
...
resource "yandex_alb_load_balancer" "alb1" {
  name               = "alb1"
  network_id         = yandex_vpc_network.network1.id
  security_group_ids = [yandex_vpc_security_group.andrey-balancer.id]
...
resource "yandex_vpc_security_group" "andrey-balancer" {
  name       = "andrey-balancer"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "andrey-private" {
  name       = "andrey-private"
  network_id = yandex_vpc_network.network1.id
  ...
  ingress {
    protocol          = "ANY"
    description       = "any"
    security_group_id = yandex_vpc_security_group.andrey-gw.id
  }
...
resource "yandex_vpc_security_group" "andrey-public" {
  name       = "andrey-public"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "andrey-gw" {
  name       = "andrey-gw"
  network_id = yandex_vpc_network.network1.id
